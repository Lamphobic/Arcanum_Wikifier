...
def furniture_wiki():
	...
	with open("furnitures.txt", "w", encoding="UTF-8") as wiki_dump:
		wiki_dump.write("\n==Gold==\n")
		wiki_dump.write("List of every item used to get more max gold:\n")
		wiki_dump.write(wiki.make_table(table_keys, table_lines, table_filter=[[5, "'gold.max' in cell"]]))

		wiki_dump.write("\n==Research==\n")
		wiki_dump.write("List of every item used to get more max research:\n")
		wiki_dump.write(wiki.make_table(table_keys, table_lines, table_filter=[[5, "'scrolls.max' in cell or 'codices.max' in cell or 'tomes.max' in cell"]]))

		wiki_dump.write("\n==Source==\n")
		wiki_dump.write("List of every item used to be able to discover or use a skill:\n")
		wiki_dump.write(wiki.make_table(table_keys, table_lines, table_filter=[[2, "'source' in cell"]]))
